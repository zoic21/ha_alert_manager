...
from .const import ATTRIBUTE_SOURCES, BLUEPRINT_OVERRIDE_FIELDS, TRANSITION_SOURCES
...
def parse_rule_yaml_data(
    ...
    *,
    ...
    """Parse rule fields while enforcing YAML keys and target identity.
    ...
    allowed = _RULE_YAML_KEYS | ({"override"} if allow_override else set())
    _reject_unknown(data, allowed, prefix="rule")
    sparse = allow_override and set(data) <= {"enabled", "override"}
    if "override" in data:
        overrides = data.pop("override")
        if (
            not isinstance(overrides, dict)
            or not set(overrides) <= BLUEPRINT_OVERRIDE_FIELDS
            or set(overrides) & set(data)
        ):
            raise ValueError("Invalid blueprint YAML overrides")
        data.update(overrides)
    if sparse and defaults is not None:
        data = {**deepcopy(defaults), **data}
    if rule_id is None:
        if "id" in data:
...
def parse_rule_yaml(raw_yaml: Any, *, rule_id: str | None = None) -> Rule:
    """Parse and validate a complete standalone rule."""
    data = parse_rule_yaml_data(raw_yaml, rule_id=rule_id)
    try:
