...
class StartupReconciliationTransaction:
    ...
    def record_removed(self, alert_id: str) -> None:
        """Forget restored provenance when a live record is authoritatively removed."""
    ...
    def committed_unverified_alert_ids(
        ...
        return set(
            self.current_alert_ids_for_originals(
                self._unverified_original_ids,
                records,
            )
        )

    def current_alert_ids_for_originals(
        self,
        original_ids: Iterable[str],
        records: dict[str, AlertRecord],
    ) -> frozenset[str]:
        """Map selected restored occurrences to their retained live identities."""
        selected = frozenset(original_ids)
        return frozenset(
            alert_id
            for alert_id, original_id in self._live_original_ids.items()
