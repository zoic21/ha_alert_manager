...
Chaque pack possède un volet **Configurer** pour ses réglages, étiquettes et exceptions par équipement/entité. Les délais et paramètres héritent indépendamment selon **entité → équipement → pack**. Effacer une valeur numérique rétablit l’héritage ; un délai explicitement à zéro est immédiat. L’interrupteur de surveillance est activé par défaut. Un pack ou équipement désactivé bloque ses exceptions d’entité ; aucune exception ne contourne la surveillance globale, une étiquette d’exclusion ou les critères d’éligibilité. Seuils de batterie et nombres de cycles consécutifs en erreur suivent la même hiérarchie ; les erreurs d’automatisation/script n’ont aucun délai par défaut.

Dans **Configuration → Général**, les exclusions globales utilisent les étiquettes Home Assistant des entités ou équipements et ne concernent que la surveillance automatique. Les règles personnalisées restent indépendantes. Les délais de déclenchement appartiennent aux packs ; le délai séparé d’affichage des alertes en attente ne contrôle que leur visibilité. Les exceptions d’équipement couvrent aussi les nouvelles entités éligibles.

Les exceptions se replient en résumés avec commandes de surveillance et suppression. Désactiver une exception masque ses paramètres sans effacer leurs valeurs. Le bouton Enregistrer principal sauvegarde les réglages de page et l’activation des packs ; chaque volet possède son propre bouton. Un volet inachevé ne bloque pas l’enregistrement de la page et sa modification ne fait pas apparaître le bouton général.

**Configurer cette surveillance**, dans les détails d’une alerte automatique, ouvre sa cible et sa source de flapping dans un brouillon non enregistré. Les cibles absentes restent disponibles pour correction ou suppression. Désactiver une cible annule délais, notifications en attente et rappels sans notification de résolution. Modifier un délai en attente conserve la date d’observation initiale ; les alertes actives/acquittées compatibles gardent leur identité.

Le flapping est désactivé par défaut. Ses réglages sont **5 occurrences en 1 heure**, puis un retour à la normale après **30 minutes sans nouvelle occurrence**. Il peut détecter les anomalies brèves qui disparaissent avant le délai normal. Les sources présélectionnées sont l’indisponibilité et la connectivité ; leurs packs doivent être activés. Les réglages sont adaptables au niveau du pack, par pack source, par équipement, par entité ou par règle personnalisée. Les détails conservent les dates des occurrences dans une section repliable, y compris dans l’historique résolu.
...
### Générer des règles depuis des blueprints

Dans **Règles personnalisées → Générer des règles**, sélectionnez les blueprints intégrés puis créez les règles sélectionnées. Le générateur affiche le nombre d’entités compatibles et explique les choix indisponibles. L’utilisation CPU/mémoire et la température CPU couvrent System Monitor et les capteurs d’équipement compatibles, dont UniFi, avec vérification des noms et unités ; les applications/add-ons Supervisor sont exclus. L’utilisation disque reste limitée à System Monitor. Activez d’abord les capteurs nécessaires.

Chaque sélection crée une règle éditable contenant jusqu’à 50 entités compatibles. Les règles sélectionnées sont créées ensemble ou pas du tout. Elles restent ensuite indépendantes : aucune synchronisation automatique ni analyse périodique n’est ajoutée. Le renommage conserve l’origine du blueprint. Régénérer une règle existante demande confirmation, remplace ses personnalisations par les valeurs par défaut et les entités actuellement découvertes, et conserve son identifiant.

L’[index des blueprints](blueprints/README.md) détaille prérequis, découverte, valeurs par défaut et limites, dont les vérifications de sauvegardes/mises à jour Home Assistant. Le [guide de contribution](rule-blueprints.md) décrit leur format YAML. Ces documents sont en anglais.
...
Un profil peut couvrir toutes les alertes ou correspondre à **au moins une étiquette sélectionnée (OU)**. La sélection combine les étiquettes d’entité, d’équipement, de règle et de pack. Une exception exige **toutes ses étiquettes (ET)**. La **première exception correspondante dans l’ordre de la liste** gagne ; les valeurs héritées conservent les réglages du profil. Les exceptions peuvent adapter séparément nouvelles alertes, résolutions et rappels. L’éditeur visuel utilise des interrupteurs explicites et un intervalle de rappel ; effacer cet intervalle désactive les rappels. Les exceptions partielles affichent les valeurs effectives du profil et le YAML accepte les champs omis pour hériter. En YAML, les exceptions utilisent `selector_ids` ; l’ancien `selector_id` reste accepté.
...
Les cibles Companion reconnues reçoivent les métadonnées natives `notification_icon` et un titre sans emoji ; l’affichage réel de l’icône dépend du client. Les cibles génériques conservent les préfixes 🚨 nouvelle alerte, 🔔 rappel et ✅ résolution. Un appui sur une notification Companion compatible ouvre l’alerte unique en cours, l’Accueil pour plusieurs alertes ou l’Historique pour les résolutions. Aucune URL brute n’est ajoutée pour les cibles génériques.
...
Chaque volet de pack propose un **Mode YAML** dans son menu trois points. Sa clé `pack` contient la configuration complète, dont les exceptions partielles `device_overrides`, `entity_overrides` et celles du flapping par source. Les durées sont en secondes et les deux éditeurs utilisent la validation backend. Cibles dupliquées, clés inconnues et réglages invalides bloquent l’enregistrement. Les profils ont leur propre éditeur YAML. Le changement de mode conserve les valeurs non enregistrées et l’ordre des listes ; fermer un éditeur modifié demande confirmation. Les volets sont redimensionnables sur ordinateur ; détails et diagnostics restent en lecture seule.
...
Une configuration enregistrée invalide entraîne un démarrage avec des valeurs sûres, une alerte persistante et le choix d’une sauvegarde par un administrateur ; aucune restauration n’est silencieuse. **Restaurer une sauvegarde complète remplace la configuration et réévalue les états actuels en conservant les identifiants compatibles d’alertes/règles, acquittements, occurrences et historique.** Les exports de version 2 conservent les identifiants de règles.

### Migration de configuration

Les anciennes configurations, YAML de version 1 et sauvegardes convertissent les délais partagés en valeurs explicites par pack et les délais par entité en exceptions, même dans les packs désactivés. Seuils de batterie, nombres de cycles consécutifs en erreur et priorités de flapping par source conservent leur comportement.

Les exclusions directes d’entités/équipements deviennent des exceptions désactivées dans chaque pack, sans perdre seuils ni délais. Les exclusions globales par étiquette sont conservées ; aucune étiquette ni entrée de registre Home Assistant n’est créée ou modifiée. Les cibles absentes restent dans les exceptions pour demeurer exclues à leur retour. Des données sources invalides laissent la configuration initiale récupérable.
