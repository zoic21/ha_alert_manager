...
test("automatic rendering is pure and places defaults with sparse exceptions", () => {
  ...
  assert.match(markup, /<\/section><\/div><div class="side-drawer-backdrop/);
...
test("settings rendering consumes prepared drafts without initializing them", () => {
  ...
  assert.equal(markup.match(/data-action="scroll-settings-section"/g)?.length, 6);
  assert.equal(markup.match(/appearance="outlined" data-action="scroll-settings-section"/g)?.length, 6);
  assert.equal(markup.match(/<ha-icon slot="start" icon="mdi:/g)?.length, 6);
  assert.match(markup, /data-section-id="general"/);
  assert.doesNotMatch(markup, /data-section-id="(?:alert-display|history|miscellaneous|exclusions)"/);
  const generalCard = markup.match(/<ha-card id="settings-section-general"[\s\S]*?<\/ha-card>/)[0];
  assert.match(generalCard, /class="settings-grid"/);
  assert.match(generalCard, /id="pending-display-delay"/);
  assert.match(generalCard, /id="history-limit"/);
  ...
  assert.ok(markup.indexOf("settings-section-general") < markup.indexOf(automaticMarkup));
  assert.ok(markup.indexOf('data-section-id="general"') < markup.indexOf('data-section-id="automatic"'));
  assert.match(generalCard, /id="excluded-labels"/);
  assert.equal((markup.match(/id="excluded-labels"/g) ?? []).length, 1);
  assert.doesNotMatch(markup, /settings-section-(?:exclusions|miscellaneous)/);
