...
Les cibles Home Assistant Companion reconnues reçoivent des icônes natives : `mdi:alert-circle` pour les nouvelles alertes, `mdi:bell-ring` pour les rappels et `mdi:check-circle` pour les retours à la normale, y compris pour les lots. Leurs titres conservent le texte et les compteurs, sans smiley redondant. Les autres cibles, les groupes de notification et les cibles Companion dont l’action mobile ne peut pas être identifiée conservent les **🚨 nouvelles alertes**, les **🔔 rappels** et les **✅ retours à la normale** dans leurs titres. Dans un profil regroupant différents types de cibles, chacune reçoit sa propre présentation.

Avec une cible Companion compatible, toucher une notification ouvre le détail d’une alerte en cours unique, la vue d’ensemble pour plusieurs alertes en cours ou l’**Historique** pour les retours à la normale. L’envoi générique transmet le titre et le message sans ajouter d’URL de navigation brute au texte.

D’après la [documentation Companion](https://companion.home-assistant.io/docs/notifications/notifications-basic/#notification-icon-and-color), Android utilise l’icône de la barre d’état et iOS un avatar d’expéditeur ; le rendu diffère selon la plateforme. **Tester** envoie `mdi:bell-check` par le même mécanisme, sans créer d’alerte ni d’entrée d’historique. Le rendu et la navigation restent à valider sur des appareils iOS/Android physiques ; aucune version minimale testée de l’application n’est annoncée. Home Assistant ne reçoit pas de confirmation du rendu : une ancienne application peut ignorer l’icône sans erreur, ce qui empêche un repli automatique vers le smiley sur cet appareil. Aucun renvoi n’est effectué uniquement pour changer une icône.
