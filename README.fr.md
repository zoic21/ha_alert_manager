...
Les opérations **Valeur** (`source: value`), **Variation** (`source: value_variation`) et **Transition** (`source: value_transition`) partagent un champ `attribute` facultatif : vide ou absent, il cible l’état ; renseigné, il cible cet attribut. Un attribut absent de l’entité ne revient jamais à l’état. Les chemins imbriqués sont acceptés ; les jokers sont réservés aux comparaisons classiques. Jinja et Aucun changement conservent leur comportement.

Les anciennes sources sont migrées automatiquement dans le stockage et les imports YAML : `state`/`attribute` → `value`, `variation`/`state_variation`/`attribute_variation` → `value_variation`, `transition`/`attribute_transition` → `value_transition`. Les attributs inutilisés des anciennes sources sur l’état sont effacés ; une ancienne source attribut sans attribut valide est rejetée. Les identifiants, alertes et historiques sont conservés. Les sources et paramètres d’affichage des alertes en cours sont migrés au chargement du stockage, avant la réconciliation du démarrage. Le moteur utilise uniquement les sources normalisées ; les anciens historiques restent inchangés.

**Transition** (`source: value_transition`, avec `attribute` facultatif) observe un passage précis `from_value` → `to_value`. Le délai de déclenchement (`duration`, 0 seconde par défaut) impose un maintien continu à la valeur d’arrivée avant activation. Quitter cette valeur annule le maintien et nécessite une nouvelle transition correspondante. Les modifications d’autres attributs ne relancent pas le délai.
...
Chaque pack dispose d’un seul volet **Configuration** regroupant ses valeurs par défaut, ses étiquettes et ses exceptions par appareil ou entité. Sa carte résume les valeurs par défaut et le nombre d’exceptions. Le switch **Surveiller** est activé par défaut. Un pack ou un appareil désactivé bloque les exceptions de ses entités. Les délais et paramètres du pack héritent indépendamment : **entité → appareil → pack**. Effacer un champ numérique rétablit l’héritage ; un délai explicite de zéro déclenche immédiatement. Les exceptions sont repliables, avec le switch de surveillance et la poubelle directement dans le résumé ; une nouvelle exception s’ouvre automatiquement. Désactiver une exception masque ses paramètres sans effacer leurs valeurs. Les exceptions d’appareil couvrent aussi les nouvelles entités compatibles. Le bouton Enregistrer général sauvegarde uniquement les réglages de la page et l’activation des packs ; chaque volet possède son propre bouton Enregistrer. Un volet incomplet reste ouvert et ne bloque pas l’enregistrement de la page. Le résultat de cet enregistrement s’affiche sur la page principale. Modifier un volet ne fait pas apparaître le bouton Enregistrer général.

Dans **Configuration → Général**, les exclusions globales utilisent les **labels Home Assistant** des entités ou appareils et concernent uniquement la surveillance automatique. Les règles personnalisées restent indépendantes. Une exception activée ne contourne ni l’interrupteur global, ni un pack désactivé, ni un label d’exclusion, ni les critères du pack. Chaque pack possède son délai de déclenchement ; le délai d’affichage des alertes à venir reste un réglage distinct.

Un administrateur peut ouvrir **Configurer cette surveillance** depuis le détail d’une alerte automatique, avec l’entité et la source d’instabilité présélectionnées. Cette ouverture ne fait aucun enregistrement. Les cibles disparues ou devenues incompatibles restent visibles pour correction ou suppression. Désactiver une cible annule ses temporisations, notifications en attente et rappels ; l’historique indique la désactivation sans notification de retour à la normale. Modifier un délai conserve le début d’observation ; les alertes actives ou acquittées compatibles gardent leur identité.
...
Les règles personnalisées (même désactivées) sont contrôlées depuis un instantané en mémoire : entités sélectionnées et références statiques des conditions/messages Jinja. Les références dynamiques et les messages en texte simple sont ignorés. Les résultats identifient le champ de la règle et le bouton Ouvrir affiche son éditeur ; ces données ne sont pas comptées comme des fichiers analysés.
...
Chaque volet de pack propose un **mode YAML** dans son menu à trois points. La clé `pack` contient l’ensemble de sa configuration, notamment les champs explicites de `device_overrides`, `entity_overrides` et les exceptions par source d’instabilité. Les durées sont en secondes. Les éditeurs visuel et YAML partagent la validation du serveur. Les cibles en double, champs inconnus ou valeurs invalides bloquent l’enregistrement ; abandonner des modifications demande confirmation. Les profils de notification conservent leur propre éditeur YAML.
...
Si la configuration enregistrée ne peut pas être chargée au démarrage, Alert Manager démarre de façon sûre avec les valeurs par défaut, affiche un avertissement persistant et laisse un administrateur choisir une sauvegarde. Aucune restauration n’est effectuée silencieusement. La restauration remplace la configuration et réévalue les états actuels en conservant les identifiants compatibles des règles et alertes, les acquittements, les occurrences et l’historique. Les nouveaux exports de version 2 conservent les identifiants de règle.

### Migration des anciennes configurations

Le chargement d’une ancienne configuration, d’un YAML de version 1 ou d’une sauvegarde convertit le délai partagé en valeurs par défaut des packs et les délais par entité en exceptions, y compris pour les packs désactivés. Les seuils de batterie et nombres d’échecs conservent leurs cibles et valeurs. Les anciens paramètres d’instabilité prioritaires par source deviennent, si nécessaire, des exceptions explicites par source pour préserver le comportement.

Les exclusions directes d’entités et d’appareils deviennent des exceptions désactivées dans chaque pack, en conservant les seuils et délais existants. Les exclusions globales par label restent inchangées. Aucun label ni registre Home Assistant n’est créé ou modifié. Les cibles absentes restent dans les exceptions pour demeurer désactivées si elles réapparaissent. Des données sources invalides laissent la configuration d’origine récupérable.
...
Les exceptions exigent que **toutes les étiquettes sélectionnées correspondent (ET)** et remplacent les réglages de nouvelle alerte, de retour à la normale ou de rappel. La **première exception correspondante dans l’ordre de la liste** est prioritaire ; l’éditeur visuel propose les mêmes switches et le même intervalle de rappel que les réglages globaux. Vider l’intervalle désactive les rappels. Les anciennes exceptions partielles affichent leurs valeurs effectives ; le YAML accepte toujours les champs absents pour hériter des valeurs du profil. En YAML, les exceptions utilisent `selector_ids` ; les anciennes exceptions à une étiquette utilisant `selector_id` restent acceptées.
...
Les détails d’une alerte distinguent les notifications d’activation et les rappels, chacun avec son nombre d’envois, ses profils correspondants et sa date de dernier envoi. Les profils réservés aux rappels apparaissent dans les rappels. Un compteur à zéro indique qu’aucun envoi de ce type n’a réussi. Un lot compte une fois par profil et par alerte, dès qu’au moins une cible a été notifiée ; les tests et les échecs complets sont exclus. L’historique conserve également les envois de retour à la normale, leurs profils et leur dernière date. Les anciens totaux cumulant activation et rappels restent dans Activation. Ces informations survivent aux redémarrages ; elles sont masquées pour les alertes à venir et les anciens envois ne sont pas reconstitués. Les notifications envoyées par des automatisations externes ne sont pas comptabilisées.
