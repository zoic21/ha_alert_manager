...
Chaque profil peut activer **Nouveaux soucis de cohérence** (`notify_on_coherence: true` en YAML, désactivé par défaut). Les contrôles lancés par le bouton de cohérence ou la planification automatique envoient un résumé des seuls nouveaux soucis, avec un lien vers Cohérence pour les destinataires compatibles. Les étiquettes et exceptions du profil ne s’appliquent pas. Un contrôle lancé depuis la page Cohérence actualise silencieusement le rapport de comparaison. Le premier contrôle considère ses résultats comme nouveaux ; un souci disparu puis retrouvé est à nouveau signalé. Les sources illisibles ou ignorées conservent leur référence de comparaison. Les demandes simultanées partagent le contrôle et l’origine du premier appel. Les envois réussis comptent dans l’utilisation des profils ; aucune alerte ni aucun rappel n’est créé.
