...
Chaque pack dispose d’un seul volet **Configuration** regroupant ses valeurs par défaut, ses étiquettes et ses exceptions par appareil ou entité. Sa carte résume les valeurs par défaut et le nombre d’exceptions. Seuls les champs explicitement modifiés sont enregistrés : **entité → appareil → pack**, indépendamment pour la surveillance, le délai et les paramètres du pack. Effacer un champ rétablit l’héritage ; un délai explicite de zéro déclenche immédiatement. Le volet indique la valeur héritée et son origine. Désactiver une exception conserve ses paramètres. Les exceptions d’appareil couvrent aussi les nouvelles entités compatibles.
