...
Les exclusions directes deviennent le label dédié **Alert Manager - migrated automatic exclusions**, ajouté aux labels existants et sélectionné comme exclusion automatique globale. Les écritures des registres sont terminées avant de retirer les anciennes exclusions. Une reprise après interruption réutilise le même label identifié. Si une cible manque ou n’appartient pas au registre, si un label utilisateur porte déjà ce nom, ou si une opération échoue, la conversion s’arrête avec une erreur précise et les données d’origine restent récupérables. Rétablissez la cible ou corrigez/supprimez explicitement son exclusion dans le YAML d’origine, puis importez à nouveau. Ne supprimez pas le label de migration tant que ses exclusions sont nécessaires.
